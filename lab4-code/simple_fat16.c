...
int dir_entry_create(FAT16 *fat16_ins,int sectorNum,int offset,char *Name, BYTE attr, WORD firstClusterNum,DWORD fileSize){
  ...
  /* File update time */
  /* 时间部分一定要阅读实验文档!!! */
  WORD temp_date = (time_ptr->tm_year - 80) * 512 + time_ptr->tm_mon * 32 + time_ptr->tm_yday;
...
int fat16_unlink(const char *path){
  ...
  DIR_ENTRY Dir;
  //释放使用过的簇
  char *pathInput = malloc(strlen(path) * sizeof(char));
  memcpy(pathInput, path, strlen(path));
  if(find_root(fat16_ins,&Dir,pathInput) == 1){
    return 1;
  }
  ClusterN = Dir.DIR_FstClusLO;
  /* 代码开始 */ 
  ...
  //char *pathInput = malloc(strlen(path) * sizeof(char));
  memcpy(pathInput, path, strlen(path));	
  char **orgpaths = org_path_split(pathInput);
  int pathDepth = 0, j;
  for (j = 0; path[j] != '\0'; j++)
  {
    (path[j] == '/') ? pathDepth++ : 0;
  ...
  if (strcmp(prtPath, "/") == 0){
    ...
    { 
      ...
      if (strcmp(Name, orgpaths[pathDepth-1]) == 0)
      { 
        findFlag = 1;
        offset = ((i -1) % 16) * BYTES_PER_DIR; //the offset of the current entry to be deleted
        break;
      }
      if (i % 16 == 0 && i != fat16_ins->Bpb.BPB_RootEntCnt)
      {  
